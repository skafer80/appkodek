...
<body>
    ...
    @php
        $totOreSenzaStage = collect($moduli ?? [])
            ->flatMap(fn ($modulo) => $modulo->conoscenze ?? [])
            ->filter(fn ($conoscenza) => strcasecmp(trim((string) ($conoscenza->nomeConoscenza ?? '')), 'STAGE') !== 0)
            ->sum(fn ($conoscenza) => (int) ($conoscenza->oreConoscenza ?? 0) + (int) ($conoscenza->oreFadConoscenza ?? 0));
    @endphp
    ...
    <div class="sheet">
        ...
        <div class="print-page">
            <header class="doc-header">
                <h1 class="doc-title">Scheda classe {{ $enteSelezionato ?: '-' }} - {{ $classeMeta?->nome ?: '-' }}
                </h1>
                <p class="meta"><strong>Priorita:</strong> {{ $classeMeta?->ordine + 1 }}</p>
                <p class="meta"><strong>ID KODEK:</strong> {{ $classeId ?: '-' }}</p>
                <p class="meta"><strong>Sede:</strong> {{ $classeMeta?->sede ?: '-' }}</p>
                <p class="meta"><strong>Descrizione:</strong> {{ $classeMeta?->descrizione ?: '-' }}</p>
            ...
            <section class="section">
                <h2 class="section-title">Personale non Docente</h2>
                <div class="section-body">
            ...
            <section class="section">
                ...
                <div class="section-body">
                    @if ($datiClasse)
                        @php
                            $sedeStage = trim(
                                ($datiClasse->indirizzo ?? '') .
                                    ($datiClasse->numeroCivico ?? '' ? ', ' . $datiClasse->numeroCivico : '') .
                                    ($datiClasse->comune ?? ''
                                        ? ' - ' .
                                            $datiClasse->comune .
                                            ($datiClasse->provincia ?? '' ? ' (' . $datiClasse->provincia . ')' : '')
                                        : ''),
                            );
                        @endphp
                        <table>
            ...
            <section class="section">
                <h2 class="section-title">FASCIA DOCENZA</h2>
                <div class="section-body">
                    @if ($datiClasse)
                        <table>
                            <tbody>
                        ...
                        <div style="margin-top: 10px; display: grid; gap: 8px;">
                            @foreach ($moduli as $modulo)
                                ...
                                <div style="border: 1px solid #d1d5db; border-radius: 8px; overflow: hidden;">
                                    <div
                                        style="padding: 6px 8px; background: #f9fafb; border-bottom: 1px solid #d1d5db;">
                                        <strong>{{ $modulo->nomeModuli ?: 'Modulo senza nome' }}</strong>
                                        <span style="margin-left: 10px; color: #000000;">Conoscenze:
                                            {{ count($modulo->conoscenze ?? []) }}</span>
                                        <span style="margin-left: 10px; color: #000000;">Ore:
                                            {{ $totOre }}</span>
                                        <span style="margin-left: 10px; color: #000000;">Ore FAD:
                                            {{ $totOreFad }}</span>
                                    </div>
