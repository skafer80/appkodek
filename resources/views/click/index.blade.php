        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <ul class="nav nav-tabs" id="classificaTabs" role="tablist">
                    ...
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" id="personale-tab" data-bs-toggle="tab"
                            data-bs-target="#personale-tab-pane" type="button" role="tab"
                            aria-controls="personale-tab-pane" aria-selected="false">
                            <i class="fa-solid fa-book"></i> Classifica Personale Non Docente
                        </button>
                    </li>
                    <li class="nav-item" role="presentation">
                                                            ...
                                                            <tbody>
                                                                @foreach ($giocateDelGiorno->sortBy('tempo') as $giocata)
                                                                    <tr>
                                                                        ...
                                                                        <td><a
                                                                                href="{{ route('click.showClasse', ['id' => $giocata->id]) }}">{{ $giocata->nome }}</a>
...
$giocatePerGiornoModuli = $giocateModuli->groupBy(function ($item) {
    return $item->start_time ? $item->start_time->format('d/m/Y') : 'Senza data';
                                                        ...
                                                        <tbody>
                                                            @foreach ($giocateDelGiorno->sortBy('tempo') as $giocata)
                                                                <tr>
                                                                    ...
                                                                    <td><a
                                                                            href="{{ route('click.showModuli', ['id' => $giocata->id]) }}">{{ $giocata->nome }}</a>
                                                                        @if ($giocata->software === 1)
                                                                            <span class="fa-solid fa-plane"
                                                                                style="color: bronze;"></span>
                                                                        @endif
            ...
            {{-- Tab Personale --}}
            <div class="tab-pane fade" id="personale-tab-pane" role="tabpanel" aria-labelledby="personale-tab"
                tabindex="0">
                <div class="mt-3">
                    @php
                        // Raggruppa per giorni e mantieni l'ordine cronologico inverso (più recente prima)
$giocatePerGiornoPersonale = $giocatePersonale->groupBy(function ($item) {
    return $item->start_time ? $item->start_time->format('d/m/Y') : 'Senza data';
                        });
                    @endphp

                    @forelse($giocatePerGiornoPersonale as $giorno => $giocateDelGiorno)
                        @if ($loop->first)
                            {{-- Primo accordion (più recente) - aperto di default --}}
                            <div class="accordion mb-4" id="accordionClassificaPersonale">
                                <div class="accordion-item">
                                    <h2 class="accordion-header" id="headingPersonale{{ $loop->iteration }}">
                                        <button class="accordion-button" type="button" data-bs-toggle="collapse"
                                            data-bs-target="#collapsePersonale{{ $loop->iteration }}"
                                            aria-expanded="true" aria-controls="collapsePersonale{{ $loop->iteration }}">
                                            <strong>{{ $giorno }}</strong>
                                            <span class="badge bg-primary ms-2">{{ $giocateDelGiorno->count() }}
                                                Allenamenti</span>
                                            @php
                                    ...
                                    <div id="collapsePersonale{{ $loop->iteration }}"
                                        class="accordion-collapse collapse show"
                                        aria-labelledby="headingPersonale{{ $loop->iteration }}"
                                        data-bs-parent="#accordionClassificaPersonale">
                                        <div class="accordion-body p-0">
                                            <div class="table-responsive">
                                                <table class="table table-striped table-hover align-middle mb-0">
                                                    <thead class="table-dark">
                                                    ...
                                                    <tbody>
                                                        @foreach ($giocateDelGiorno->sortBy('tempo') as $giocata)
                                                            <tr>
                                                                ...
                                                                <td><a
                                                                        href="{{ route('click.showClassePersonale', ['id' => $giocata->id]) }}">{{ $giocata->nome }}</a>
                                                                    @if ($giocata->software === 1)
                                                                        <span class="fa-solid fa-plane"
                                                                            style="color: bronze;"></span>
                                                                    @endif
                                ...
                                <div class="accordion-item">
                                    <h2 class="accordion-header" id="headingPersonale{{ $loop->iteration }}">
                                        <button class="accordion-button collapsed" type="button"
                                            ...
                                            data-bs-target="#collapsePersonale{{ $loop->iteration }}"
                                            aria-expanded="false"
                                            aria-controls="collapsePersonale{{ $loop->iteration }}">
                                            <strong>{{ $giorno }}</strong>
                                            <span class="badge bg-primary ms-2">{{ $giocateDelGiorno->count() }}
                                                Allenamenti</span>
                                            @php
                                                $tempoMedio = $giocateDelGiorno->whereNotNull('tempo')->avg('tempo');
                                            @endphp
                                            @if ($tempoMedio)
                                                <span class="badge bg-success ms-2">Media:
                                                    {{ floor($tempoMedio / 60) }}:{{ str_pad($tempoMedio % 60, 2, '0', STR_PAD_LEFT) }}</span>
                                    ...
                                    <div id="collapsePersonale{{ $loop->iteration }}" class="accordion-collapse collapse"
                                        aria-labelledby="headingPersonale{{ $loop->iteration }}"
                                        data-bs-parent="#accordionClassificaPersonale">
                                        <div class="accordion-body p-0">
                                            <div class="table-responsive">
        ...
        <div class="tab-pane fade" id="media-tab-pane" role="tabpanel" aria-labelledby="media-tab"
            ...
            <div class="mt-3">
                @php
                    // Raggruppa per giorni e mantieni l'ordine cronologico inverso (più recente prima)
$giocatePerGiornoMedia = $giocateConMedia->groupBy(function ($item) {
    return $item->start_time ? $item->start_time->format('d/m/Y') : 'Senza data';
                    });
                @endphp
                                            <div class="table-responsive">
